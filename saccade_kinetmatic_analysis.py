...
# Start with computing saccadic onset latencies for first saccades only
	...
	for ax,subject_dists,subject_mew,subject_early_crit,subject_late_crit,i in zip(flatten(ax_arrs1),onset_latencies,mew_latencies,early_latency_crit,late_latency_crit,
		...
		ax.set_ylim([0,100]); ax.set_xlim([0,750]);  #1000
		#format the axis
		if (i==10):  #(i==0)|(i==5)|
		...
		ax.hist(subject_dists, bins = [0,50,100,150,200,250,300,350,400,450,500,550,600,650,750]);
		...
		ax.text(525, 80, '# saccades: %s'%(len(subject_dists)), size = 8)
	...
	for ax1,subject_dists,subject_mew,subject_early_crit,subject_late_crit,i in zip([f for f in flatten(ax_arrs2)][0:15],onset_latencies[15:],mew_latencies[15:],early_latency_crit[15:],late_latency_crit[15:],
		...
		# set axis limits the same for easy comparison
		ax1.set_xlim([0,750]); ax1.set_ylim([0,100]); 
		#format the axis
		if (i==10):  #(i==0)|(i==5)|
		...
		#now plot the distributions, then add the mean and cutoffs	
		ax1.hist(subject_dists, bins = [0,50,100,150,200,250,300,350,400,450,500,550,600,650,750]);
...
#1. Create data holders for the analyses
	...
	subj_proportion_arrays = [zeros((40,40)) for su in block_matrix];
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		for b in blocks:
			...
			#check if there are no instances of first saccades that are too fast
			if m == 0:
				#in this case, reassign the arrray as zeros
				subj_proportion_arrays[subj_nr] = zeros((40,40));
			...
			if b.block_nr==len(blocks):
				...
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('ALLTRIALS_STARTINGPOSNOTFIXATION_heatmap_subj_%s'%(subj_nr));
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	for su in zip(subj_proportion_arrays):
		agg_prop_array += su[0]; #add each subjects' heat maps together		
	agg_prop_array = agg_prop_array/len(block_matrix); #to get the average
	
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('ALLTRIALS_STARTINGPOSNOTFIXATION_heatmap_subj_%s'%('ALLSUBJECTS'));
...
#this calculates the heat maps for all first saccades, including fast latency first saccades
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('%s_FIRST_SACCADE_heatmap_subj_%s'%(name, subj_nr));
				#set a legend. first, get the maximal value in the array to define a legend
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);
				#add text to say how many saccade (total and max)		
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('%s_FIRST_SACCADE_heatmap_subj_%s'%(name,'ALLSUBJECTS'));
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades:\n %s '%(sum(agg_array)),size=8, color = 'white');
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('ALLTRIALTYPES_FIRST_SACCADE_heatmap_subj_%s'%(subj_nr));
				#set a legend. first, get the maximal value in the array to define a legend
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('ALLTRIALTYPES_FIRST_SACCADE_heatmap_subj_%s'%('ALLSUBJECTS'));
	#set a legend. first, get the maximal value in the array to define a legend
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
			...
			for i in arange(0,len(b.trials)):
				...
				if ((b.trials[i].dropped_sample == 0)&(b.trials[i].didntLookAtAnyItems == 0)&
					...
					if (b.trials[i].nr_saccades > 0): #this conditional is used to ensure that no trials without saccades sneak through
						...
						#for this analysis, I only want to build a heat map for first saccades with very fast onsets (less than  critical value)
						if sac_start_time<=early_lat:				
							#now check where in the spatial array is the closest distane to the X,Y position of this data point
							#this will be the minimum of the distance between each xx point and yy point
							x_x, y_y = meshgrid(xx_vec, yy_vec);
							minimum = 10000; coors = array([nan,nan]); #this pre-allocates a very large minimum and an array to hold the indices for the spatial position array
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, data in enumerate(zip(block_matrix,early_latency_crit)):
		...
		for b in blocks:
			for i in arange(0,len(b.trials)):
				...
				if ((b.trials[i].dropped_sample == 0)&(b.trials[i].didntLookAtAnyItems == 0)&
					...
					if (b.trials[i].nr_saccades > 0): #this conditional is used to ensure that no trials without saccades sneak through
						...
						#for this analysis, I only want to build a heat map for first saccades with very fast onsets (less than criterion value)
						if sac_start_time<=early_lat:				
							#now check where in the spatial array is the closest distane to the X,Y position of this data point
							#this will be the minimum of the distance between each xx point and yy point
							x_x, y_y = meshgrid(xx_vec, yy_vec);
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); ax = imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('FASTLATENCY_ALLTRIALTYPES_FIRST_SACCADE_heatmap_subj_%s'%(subj_nr));
				#set a legend. first, get the maximal value in the array to define a legend
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);
				
				#add text to say how many saccade (total and max)		
				f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
				f.text(0.25, 0.8, 'Max nr of saccades: %s \n'%(m),size=8, color = 'white');
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('FASTLATENCY_ALLTRIALTYPES_FIRST_SACCADE_heatmap_subj_%s'%('ALLSUBJECTS'));
	#set a legend. first, get the maximal value in the array to define a legend
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);

	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(agg_array)),size=8, color = 'white');
	f.text(0.25, 0.8, 'Max nr of saccades: %s \n'%(m),size=8, color = 'white');
...
def createFirstSaccadeEndpointMapNoFastLatencyAllTrialTypesTogether(block_matrix):
# This function does the same as above, but does so by collapsing across trial types

	start_time = time.time(); #start recording how long this takes

#0. Get the early onset latencies, and then use thse to filter only saccades with greater than that trials
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, data in enumerate(zip(block_matrix, early_latency_crit)):
		...
		for b in blocks:
			for i in arange(0,len(b.trials)):
				...
				if ((b.trials[i].dropped_sample == 0)&(b.trials[i].didntLookAtAnyItems == 0)&
					...
					if (b.trials[i].nr_saccades > 0): #this conditional is used to ensure that no trials without saccades sneak through
						...
						if sac_start_time>early_lat:  #use this to filter the fast latency saccades here
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('ALLTRIALTYPES_FIRST_SACCADE_NOEARLYONSET_heatmap_subj_%s'%(subj_nr));
				#set a legend. first, get the maximal value in the array to define a legend
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);
				...
				#f.text(0.25, 0.85, 'Total nr of saccades:\n %s '%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
				#f.text(0.25, 0.8, 'Max nr of saccades:\n %s '%(m),size=8, color = 'white');
				savefig(figurepath+'heatmaps/SACCADE_HEATMAPS/'+'ALLTRIALTYPES_FIRSTSACCADE_NOEARLYONSET_heatmap_subj_%s.png'%(subj_nr));
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('ALLTRIALTYPES_FIRST_SACCADE_NOEARLYONSET_heatmap_subj_%s'%('ALLSUBJECTS'));
	#set a legend. first, get the maximal value in the array to define a legend
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades:\n %s '%(sum(agg_array)),size=8, color = 'white');
	f.text(0.25, 0.8, 'Max nr of saccades:\n %s '%(m),size=8, color = 'white');		
	savefig(figurepath+'heatmaps/SACCADE_HEATMAPS/'+'ALLTRIALTYPES_FIRSTSACCADE_NOEARLYONSET_heatmap_subj_%s.png'%('ALLSUBJECTS'));
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		...
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('%s_OTHER_SACCADES_heatmap_subj_%s'%(name, subj_nr));
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);		
				#add text to say how many saccade (total and max)		
				f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('%s_OTHER_SACCADE_heatmap_subj_%s'%(name, 'ALLSUBJECTS')); 
	#set a legend. first, get the maximal value in the array to define a legend
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		...
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('ALLTRIALTYPES_OTHER_SACCADES_heatmap_subj_%s'%(subj_nr));
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);		
				#add text to say how many saccade (total and max)		
				f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('ALLTRIALTYPES_OTHER_SACCADE_heatmap_subj_%s'%('ALLSUBJECTS')); 
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(agg_array)),size=8, color = 'white');
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		...
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('%s_ALL_SACCADES_heatmap_subj_%s'%(name, subj_nr));
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);		
				#add text to say how many saccade (total and max)		
				f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('%s_ALL_SACCADE_heatmap_subj_%s'%(name,'ALLSUBJECTS'));
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(agg_array)),size=8, color = 'white');
...
#2. Iterate through for each subject and get the ending positions of each first saccade, storing appropriately, then create individual maps	
	...
	for subj_nr, blocks in enumerate(block_matrix):
		...
		for b in blocks:
			...
			if b.block_nr==len(blocks):
				#save the 'raw' heat maps
				f = figure(); imshow(subj_proportion_arrays[subj_nr], cmap='hot'); title('ALLTRIALTYPES_ALL_SACCADES_heatmap_subj_%s'%(subj_nr));
				mx = round(max(map(max,subj_proportion_arrays[subj_nr])),1);
				cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3)); cb.outline.set_linewidth(2.0);		
				#add text to say how many saccade (total and max)		
				f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(subj_arrays[subj_nr])),size=8, color = 'white');
...
# 3. Aggregate across subjects by finding the average fixation accumulation		
	...
	#create and save the figure
	f = figure(); imshow(agg_prop_array, cmap='hot'); title('ALLTRIALTYPES_ALL_SACCADE_heatmap_subj_%s'%('ALLSUBJECTS'));
	mx = max(map(max,agg_prop_array));
	cb = colorbar(pad = 0.1, ticks = linspace(0,mx,3), format = '%2.1f'); cb.outline.set_linewidth(2.0);
	m = round(max(map(max,agg_array)),1);
	f.text(0.25, 0.85, 'Total nr of saccades: %s \n'%(sum(agg_array)),size=8, color = 'white');
